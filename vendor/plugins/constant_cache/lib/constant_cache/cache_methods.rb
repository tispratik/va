  ...
  module CacheMethods #:nodoc:
    ...
    module ClassMethods
      ...
      def caches_constants(additional_options = {})
        ...
        if additional_options[:return_column].nil?
          all.each {|model| model.set_instance_as_constant_return_id}
        elsif additional_options[:return_column] == "object"
          all.each {|model| model.set_instance_as_constant_return_obj}
        else
          all.each {|model| model.set_instance_as_constant_return_other(additional_options[:return_column])}
        end  
      end
    end
    ...
    module InstanceMethods
      ...
      #
      def set_instance_as_constant_return_id
        const = constant_name
        if !const.blank?
          ...
          self.class.const_set(const, self.id) if !const.blank?
        end
      end
      
      def set_instance_as_constant_return_other(return_value)
        const = constant_name
        if !const.blank?
          if self.class.const_defined?(const)
            message = "Constant #{self.class.to_s}::#{const} has already been defined"
          ...
          self.class.const_set(const, eval("self.#{return_value.to_s}")) if !const.blank?
        end
      end
      
      def set_instance_as_constant_return_obj
        const = constant_name
        if !const.blank?
          if self.class.const_defined?(const)
            message = "Constant #{self.class.to_s}::#{const} has already been defined"
