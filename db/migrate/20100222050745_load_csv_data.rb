...
class LoadCsvData < ActiveRecord::Migration
  def self.up
    ...
    ind = nil
    puts "1. Pcsales"
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcsales.csv",
                  :headers           => true,
    ...
    line = nil
    puts "2. Pcproducts"
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcproducts.csv",
                  :headers           => true,
                  :header_converters => :symbol ) do |line|
    ...
    line = nil
    puts "3. Pcfeatures"
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcfeatures.csv",
                  :headers           => true,
                  :header_converters => :symbol ) do |line|
    ...
    line = nil
    puts "4. Pcplans"
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcplans.csv",
                  :headers           => true,
                  :header_converters => :symbol ) do |line|
    ...
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcplan_pcfeatures.csv",
                  :headers           => true,
                  :header_converters => :symbol ) do |line|
    ...
    ind = nil
    puts "6. Pcbooster"
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcboosters.csv",
                  :headers           => true,
                  :header_converters => :symbol ) do |line|
    ...
    puts "9. Pcusage"
    CSV.foreach( "#{RAILS_ROOT}/db/seed/csv/pcusages.csv",
                  :headers           => true,
                  :header_converters => :symbol ) do |line|
